...
if (UI_GLOBALS::DEFAULT_LAYOUT) changeLayout();
...
if (db->connectDataBase(options))
  ...
  itemSelector->init(db->exec(SqlSelectQuery::buildSelectQuery({NAME_COLUMN,GROUPID_COLUMN,DESCRIPTION_COLUMN},DESCRIPTIONS_TABLE,{},{},false,NAME_COLUMN)),
...
QAction *openDBAction = fileMenu->addAction("Открыть БД...");
QAction *closeDBAction = fileMenu->addAction("Закрыть БД");
...
connect(aboutAction, &QAction::triggered, this, [=](){QMessageBox::about(this, "О программе",GLOBALS::ABOUT_TEXT);});
}
...
ui->queryEdit->setPlainText(SqlSelectQuery::buildUnion(SqlSelectQuery::buildSelectQuery(DEFAULT_VALS_ITEMS,VALUES_TABLE,
                                                                                        {SqlJoin(DESCRIPTIONS_TABLE,VARID,DESCRIPTIONS_ID),
                                                                                         SqlJoin(GROUPS_TABLE,GROUPID,GROUPS_ID)},
                                                                                         filters),
                                                       additional));
